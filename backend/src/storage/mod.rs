...
use crate::{config::{Config, StorageConfig}, AppError, Result};
    ...
    pub async fn new(config: &Config) -> Result<Self> {
        match &config.storage {
            ...
            StorageConfig::Gcs { .. } => {
                ...
                Err(AppError::Storage("GCS storage not yet implemented".to_string()))
            }
        }
    }
        ...
        match self {
            ...
            StorageProvider::S3 { client, bucket } => {
                ...
                let key = file_url
                    .split('/')
                    .last()
                    .ok_or_else(|| AppError::Storage("Invalid S3 URL".to_string()))?;
