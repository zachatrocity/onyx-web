...
export class Chat {
	...
	#render(effect: Effect) {
		const root = DOM.create("div", {
			className: "flex items-center gap-2 px-3 py-2 backdrop-blur-md rounded-lg transition-all ease-out transform duration-200 shadow-lg bg-black/40 fixed max-w-sm",
		});
		...
		effect.effect((effect) => {
			DOM.render(effect, root, effect.get(message));
		});

		effect.effect((effect) => {
			const message = effect.get(this.broadcast.source.chat.markdown.message);
		...
		effect.effect((effect) => {
			...
			const top = Math.round(devicePixelRatio * Math.min(canvasRect.top + canvasRect.height - root.clientHeight - 40, messageTop) / devicePixelRatio);

			// Center message horizontally on broadcast
			const messageCenterX = pageBounds.x + pageBounds.width / 2;
			const left = Math.round(devicePixelRatio * Math.min(
